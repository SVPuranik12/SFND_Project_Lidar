...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    // Creating a filtercloud objects : downsample the data set using leaf size of .2m
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    // TODO:: Fill in this function to find inliers for the cloud. Added 02.05.2020
    ...
    // Segment the largest planer component from input cloud 
     
     seg.setInputCloud(cloud);
     seg.segment(*inliers, *coefficients);
     if (inliers->indices.size() == 0)
     {
         std::cout << "Could not estimate a planer model for given data set" << std::endl;
     }
    std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult = SeparateClouds(inliers, cloud);
   */
     
    
    /* Using RANSAC Impmentation */
    /**/
    std::unordered_set<int> inliersResult; //Initialising with 0
	...
	while(maxIterations--)
	{
		...
		for (int index = 0; index < cloud->points.size(); index ++)
		{
			...
			x_ref = cloud->points[index].x;
			y_ref = cloud->points[index].y;
			z_ref = cloud->points[index].z;
    ...
    if(inliers->indices.size() ==0)
    {
        std::cout << "Could not estimat a planer model for given data set" <<std::endl;
    }
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    /* Clustering Using library functions 
   ...
   /* Clustering using KDtree algorithm */
    std::vector<typename pcl::PointCloud<PointT>::Ptr> clusters;
